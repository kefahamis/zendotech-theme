 ...
 * Fixed: null-safe link url, safe bg_style handling, proper add_action
 */
...
class Zendotech_Home_Banners_Widget extends \Elementor\Widget_Base {
	...
	public function get_name()       { return 'zendotech_home_banners'; }
	public function get_title()      { return __( 'Home Banners', 'zendotech' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'banners', [
			...
			'fields'      => [
				...
				[
					'name'    => 'title',
					'label'   => __( 'Title (HTML allowed)', 'zendotech' ),
			...
			'default'     => [
				[ 'subtitle' => 'UP TO 30% OFF', 'title' => 'Table &<br> SMARTPHONE', 'bg_color' => '#f7f7f7', 'button_text' => 'Shop now', 'link' => [ 'url' => '#' ] ],
				[ 'subtitle' => 'UP TO 30% OFF', 'title' => 'Camera &<br>Video',      'bg_color' => '#f1f1f1', 'button_text' => 'Shop now', 'link' => [ 'url' => '#' ] ],
				[ 'subtitle' => 'UP TO 30% OFF', 'title' => 'TELEVISION<br>&amp; HOME', 'bg_color' => '#ebebeb', 'button_text' => 'Shop now', 'link' => [ 'url' => '#' ] ],
			],
		...
		$this->add_responsive_control( 'card_padding', [
			...
			'size_units' => [ 'px', 'em', '%' ],
			'selectors'  => [ '{{WRAPPER}} .bwp-widget-banner .bg-banner' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};' ],
		] );
		$this->add_control( 'card_radius', [
			'label'      => __( 'Border Radius', 'zendotech' ),
			'type'       => \Elementor\Controls_Manager::SLIDER,
			'range'      => [ 'px' => [ 'min' => 0, 'max' => 50 ] ],
			'selectors'  => [ '{{WRAPPER}} .bwp-widget-banner' => 'border-radius: {{SIZE}}{{UNIT}};' ],
		] );
		$this->end_controls_section();
		...
		$this->add_control( 'subtitle_color', [
			...
			'type'      => \Elementor\Controls_Manager::COLOR,
			'selectors' => [ '{{WRAPPER}} .bwp-image-subtitle' => 'color: {{VALUE}};' ],
		] );
		...
		$this->add_control( 'title_color', [
			'label'     => __( 'Color', 'zendotech' ),
			'type'      => \Elementor\Controls_Manager::COLOR,
			'selectors' => [ '{{WRAPPER}} .title-banner' => 'color: {{VALUE}};' ],
		] );
		$this->add_group_control( \Elementor\Group_Control_Typography::get_type(), [
	...
	protected function render() {
		...
		if ( empty( $settings['banners'] ) ) return;
		?>
		<div class="zendotech-home-banners-wrap section pt-0">
			<div class="container">
				<div style="display:grid;grid-template-columns:repeat(<?php echo count( $settings['banners'] ); ?>,1fr);gap:20px;">
					<?php foreach ( $settings['banners'] as $item ) :
						...
						?>
						<div class="bwp-widget-banner layout-1"<?php if ( $card_style ) echo ' style="' . $card_style . '"'; ?>>
							<div class="bg-banner">
								<div class="banner-wrapper banners">
									<div class="bwp-image">
										<a href="<?php echo esc_url( $url ); ?>"<?php echo $target . $nofollow; ?>>
											<?php if ( ! empty( $item['image']['url'] ) ) : ?>
												<img src="<?php echo esc_url( $item['image']['url'] ); ?>" alt="<?php echo esc_attr( wp_strip_all_tags( $item['title'] ) ); ?>">
											<?php endif; ?>
										</a>
									</div>
										...
										<div class="info">
											<div class="content">
												<?php if ( ! empty( $item['subtitle'] ) ) : ?>
													<div class="bwp-image-subtitle"><?php echo esc_html( $item['subtitle'] ); ?></div>
												<?php endif; ?>
